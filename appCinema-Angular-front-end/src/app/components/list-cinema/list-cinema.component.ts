...
export class ListCinemaComponent implements OnInit {
  ...
  getFilms() {
    ...
    this.filmService.getAll().subscribe(
      resource => {
        this.films = ((JSON.parse(JSON.stringify(resource))))
      }
    )
  }
  ...
  onGetSalles(cinema) {
    ...
    this.cinemaService.get(cinema.id, "salles").subscribe(
      resource => {
        this.salles = ((JSON.parse(JSON.stringify(resource))._embedded.salles));
  ...
  reserve() {
    Swal.fire({
      ...
      confirmButtonText: 'Oui, réservez-le'
    }).then((result) => {
      if (result.value) {
        this.reserved();
      }
